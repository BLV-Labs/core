...
func (q queryServer) ProposalMinimalLUNCByUusd(ctx context.Context, req *v2lunc1types.QueryProposalRequest) (*v2lunc1types.QueryProposalResponse, error) {
	// Fetch the proposal using the proposal ID
	proposal, found := q.k.GetProposal(ctx, req.ProposalId)
	if !found {
