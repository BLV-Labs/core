...
// SetMinLunc sets a minimum Lunc based on real-time price to store.
// Panics if can't marshal the proposal.
func (keeper Keeper) SetMinLunc(ctx sdk.Context, proposal v1.Proposal) {
	bz, err := keeper.MarshalProposal(proposal)
	if err != nil {
		panic(err)
	}
	price, err := keeper.oracleKeeper.GetLunaExchangeRate(ctx, "uusd")
	if err != nil {
		return
	}

	minLunc := keeper.baseKeeper.GetParams(ctx).

	store := ctx.KVStore(keeper.storeKey).getParams.
